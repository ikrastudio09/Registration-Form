...
export default function PlayingStyleSelector({ selected, onChange, error }) {
  const handleToggle = (value) => {
    if (selected.includes(value)) {
      ...
      if (selected.length >= 2) return; // Max 2 allowed
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center justify-between">
        <label className="flex items-center gap-2" style={{ color: 'rgba(232,245,233,0.7)', fontFamily: 'var(--font-accent)', letterSpacing: '0.05em', textTransform: 'uppercase', fontSize: '11px' }}>
          <span>🏅</span> Playing Style <span style={{ color: '#f59e0b' }}>*</span>
        </label>
        <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: selected.length === 2 ? 'rgba(245,158,11,0.2)' : 'rgba(255,255,255,0.06)', color: selected.length === 2 ? '#fbbf24' : 'rgba(232,245,233,0.4)', fontFamily: 'var(--font-accent)' }}>
      ...
      {/* Warning */}
      {selected.length === 2 && (
        <p className="text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 animate-fade-in" style={{ background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.2)', color: '#fbbf24' }}>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate" style={{ color: isSelected ? 'var(--color-green)' : 'var(--color-text)', fontFamily: 'var(--font-accent)' }}>
                  {style.value}
                </p>
